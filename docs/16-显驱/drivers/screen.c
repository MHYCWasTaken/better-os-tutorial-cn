...
void kprint_at(char *message, int col, int row) {
    ...
    if (col >= 0 && row >= 0)
        ...
        row = get_row(offset); // 设定row为offset的row
        col = get_col(offset); // 设定col
    }
...
int print_char(char c, int col, int row, char attr) {
    ...
    if (col >= MAX_COLS || row >= MAX_ROWS) {
        ...
        return get_position(col, row);
    }

    int offset;
    if (col >= 0 && row >= 0) offset = get_position(col, row);
...
int get_position(int col, int row) { return 2 * (row * MAX_COLS + col); }
int get_row(int offset) { return offset / (2 * MAX_COLS); }
int get_col(int offset) { return (offset - (get_row(offset)*2*MAX_COLS))/2; }
